...
def expm_test2(DTYPE_t[:, :] A not None):
...
cdef _pade3(DTYPE_t[:, :] A):
    ...
    cdef int n = A.shape[0]
    cdef DTYPE_t[:, :] A2, U, V
    cdef DTYPE_t[:, :] ident = square_identity(n)
    A2 = np.dot(A, A)
    U = np.dot(A,
...
cdef _pade9(DTYPE_t[:, :] A):
    ...
    """
    cdef int n = A.shape[0]
    cdef DTYPE_t[:, :] A2, A4, A6, A8, U, V
    cdef DTYPE_t[:, :] ii = square_identity(n)
    A2 = np.dot(A, A)
    A4 = np.dot(A2, A2)
    A6 = np.dot(A4, A2)
    A8 = np.dot(A6, A2)
...
cdef square_identity(int n):
    ...
    """
    cdef DTYPE_t[:, :] a = np.zeros((n, n), dtype=DTYPE)
...
cdef square_mult(DTYPE_t[:, :] a, DTYPE_t m):
    """ TODO """
    cdef int n = a.shape[0]
    cdef DTYPE_t[:, :] b = np.zeros((n, n), dtype=DTYPE)
...
cdef square_add(DTYPE_t[:, :] a, DTYPE_t[:, :] b):
    """ TODO """
    cdef int n = a.shape[0]
    cdef DTYPE_t[:, :] x = np.zeros((n, n), dtype=DTYPE)
...
cdef square_add3(DTYPE_t[:, :] a, DTYPE_t[:, :] b, DTYPE_t[:, :] c):
    """ TODO """
    cdef int n = a.shape[0]
    cdef DTYPE_t[:, :] x = np.zeros((n, n), dtype=DTYPE)
...
cdef square_add4(DTYPE_t[:, :] a, DTYPE_t[:, :] b, DTYPE_t[:, :] c,
                 DTYPE_t[:, :] d):
    """ TODO """
    cdef int n = a.shape[0]
    cdef DTYPE_t[:, :] x = np.zeros((n, n), dtype=DTYPE)
...
cdef square_add5(DTYPE_t[:, :] a, DTYPE_t[:, :] b, DTYPE_t[:, :] c,
                 DTYPE_t[:, :] d, DTYPE_t[:, :] e):
    """ TODO """
    cdef int n = a.shape[0]
    cdef DTYPE_t[:, :] x = np.zeros((n, n), dtype=DTYPE)
...
"""
cdef square_dot(DTYPE_t[:, :] a, DTYPE_t[:, :] b):
    int n = a.shape[0]
    cdef DTYPE_t[:, :] x = np.zeros((n, n), dtype=DTYPE)
